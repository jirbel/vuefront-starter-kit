<template>
  <vf-m-dropdown v-if="filteredCurrency && filteredCurrency.length > 1" class="currency-section" variant="link" size="sm">
    ...
    <vf-m-dropdown-item
      v-for="(value, index) in filteredCurrency"
      :key="index"
...
export default {
  computed: {
    ...
    filteredCurrency: (vm) => vm.currency.filter(item => item.title)
  },
  methods: {
